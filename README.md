...
### 3. 科学上网指南 (`guide.html`)
- 紫色主题赛博风格（紫+青双色粒子背景）
- 分步指南：获取节点 → 下载客户端 → 导入使用
- 6 大客户端推荐（Clash Verge / V2RayN / V2RayNG / Shadowrocket / Surfboard / NekoBox）
- 常见问题 FAQ 解答
- 滚动入场动画、高亮提示框
...
- **安全**：访问口令使用 SHA-256 哈希存储，源码中无明文密码，剪贴板 API 错误兜底
- **无障碍**：语义化按钮、键盘导航支持、aria 属性、noscript 兜底
- **动画**：粒子网络、故障特效、CRT 扫描线、滚动入场、卡片渐入
- **响应式**：完全适配移动设备，字体 clamp 自适应
...
### v5.0 (2026-03)
- 全站视觉升级：所有页面增加粒子网络背景和 CRT 扫描线效果
- `index.html` 时钟升级为渐变发光效果
- `portal.html` 全面重构：语义化按钮、剪贴板兜底、键盘快捷键
- `guide.html` 完全重写：分步教程 + 客户端推荐网格 + FAQ 常见问题
- `404.html` 修复编码乱码问题，增加自动倒计时跳转
- `style.css` 扩展为完整公共样式系统（滚动条、选中色、焦点样式）
- `update_subs.py` 修复 check_url() 中的死代码 bug
- 全站增加无障碍支持（键盘导航、aria 标签、noscript 兜底）
- 全站统一 SEO/OG meta 标签和字体预加载
- 移除 `user-scalable=no` 限制
